...
class BasicDecimalInputTableViewCell: UITableViewCell, UITextFieldDelegate {

    var questionCompletion: QuestionCompletion?
    var inputTFDecimalChangedCompletion: InputTFDecimalChangedCompletion?
    ...
    private var numberMaxValue: CGFloat = 0.0
    ...
    @objc private func doneButtonClicked(_ button: UIButton) -> Void {
        ...
        if self.inputCountTF.text?.count == 0 {
            self.inputCountTF.text = String(format: "%f", self.defaultValue)
        }
        
        self.inputTFDecimalChangedCompletion?(CGFloat(Float(self.inputCountTF.text!)!))
    }
    
    /// Update UI.
    func updateUI(with title: String, valueNum: CGFloat) -> Void {
        ...
        self.inputCountTF.text = String(format: "%f", valueNum)
    }
    
    /// Setting the maxvalue of the inputCountTF.
    func setInputMaxValue(with maxValue: CGFloat) -> Void {
        self.titleOffset = 0.0
        self.questionButtonIsHidden = false
        self.numberMaxValue = maxValue
    }
    ...
    @objc private func textFieldDidChange(_ textField: UITextField) -> Void {
        ...
        let numValue = CGFloat(Float(self.inputCountTF.text ?? "") ?? 0)
        if numValue > self.numberMaxValue {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        let expression = "^[0-9]*((\\.|,)[0-9]{0,6})?$" // ^[0-9]*((\\.|,)[0-9]{0,2})?$
