...
class ToolsManager: NSObject {
    ...
    func calculateWidth(with text: String, font: UIFont, componentHeight: CGFloat) -> CGFloat {
        ...
        let frame = (text as NSString).boundingRect(with: CGSizeMake(10000, componentHeight), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: dic, context: nil)
    ...
    func calculateHeight(with text: String, font: UIFont, componentWidth: CGFloat) -> CGFloat {
        ...
        let dic = [NSAttributedString.Key.font: font]
        let frame = (text as NSString).boundingRect(with: CGSizeMake(componentWidth, 10000), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: dic, context: nil)
