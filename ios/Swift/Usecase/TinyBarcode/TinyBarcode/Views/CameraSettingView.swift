...
class CameraSettingView: UIView {
    ...
    private var autoZoomLabel: UILabel = {
        let label = UILabel.init(frame: CGRect(x: kLeftMarginOfContainer, y: 0, width: 100, height: KCameraSettingAvailableHeight))
        label.textColor = .white
        label.text = "Auto Zoom"
        label.font = kFont_SystemDefault(KCameraSettingTitleTextSize)
