...
class BasicTextInputTableViewCell: UITableViewCell, UITextFieldDelegate {
    var questionCompletion: QuestionCompletion?
    var inputTFTextChangedCompletion: InputTFTextChangedCompletion?
    ...
    private lazy var inputCountTF: UITextField = {
        let textField = UITextField.init(frame: CGRect(x: kScreenWidth - kCellRightMargin - 150, y: (kCellHeight - 20) / 2.0, width: 150, height: 20))
        ...
        textField.placeholder = "Input your RegEx"
    ...
    @objc private func doneButtonClicked(_ button: UIButton) -> Void {
        ...
        if self.inputCountTF.text?.count == 0 {
            self.inputCountTF.text = self.defaultString
        }
        self.inputTFTextChangedCompletion?(self.inputCountTF.text!)
    }
    
    /// Update UI.
    func updateUI(with title: String, valueString: String) -> Void {
        ...
        self.inputCountTF.text = valueString
    }
    
    /// Setting the maxvalue of the inputCountTF.
    func setInputMaxLength(with maxLength: Int) -> Void {
        self.titleOffset = 0.0
        self.questionButtonIsHidden = false
        self.stringMaxLength = maxLength
    }
    ...
    @objc private func textFieldDidChange(_ textField: UITextField) -> Void {
        ...
        if textLength > self.stringMaxLength {
